...
fn main() { 
    thread::spawn(move || { 
    println!("31;1", "Spawned-thread is waiting ..."); 
    ...
    thread::yield_now(); 
    println!("31;1","Main thread set FLAG to notify spawned-thread to continue."); 
    let _ = FLAG.fetch_add(1, Ordering::Relaxed); 
    println!("31;1","Main thread waits spawned-thread to respond ..."); 
    while FLAG.load(Ordering::Relaxed) < 2 { 
        thread::yield_now(); 
    }
    println!("31;1", "Preempt test run OK!"); 
}